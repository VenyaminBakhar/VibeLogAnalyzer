...
public class DeepSeekService {
    ...
    private String buildSqlGenerationPrompt(String userQuery, List<LogPattern> patterns) {
        ...
        prompt.append("Итоговый SELECT всегда должен возвращать все колонки из таблицы log_entries (SELECT id, timestamp, log_level, message...). Это необходимо для предоставления максимального контекста для анализа. Так же итоговый SELECT должен исключать дубликаты. Используй DISTINCT для этого (пример: SELECT DISTINCT id, timestamp, log_level, message...).\n\n");
